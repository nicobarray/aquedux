...
const findChannelForAction = (state: State, action: Action): Channel => {
  ...
  if (!channel) {
    throw new Error(
      `findChannelForAction -- Action ${action.type} does not match any channel. Did you call addChannel or addChannelTemplate?`
    )
  }
...
const getChannelHandlersFromAction = (state: State, action: Action): Channel => {
  ...
  if (!channel) {
    throw new Error(
      `getChannelHandlersFromAction -- Action ${action.type} does not match any channel. Did you call addChannel or addChannelTemplate?`
    )
  }

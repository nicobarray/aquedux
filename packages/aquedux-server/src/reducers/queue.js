...
const reducer = reducr.default(initialState, {
  ...
  [actionTypes.queue.AQUEDUX_ENQUEUE_NOTIFICATION]: (prevState, _action) => ({
    ...prevState,
    cursor: prevState.cursor + 1,
    notificationQueue: [...prevState.notificationQueue, prevState.cursor]
  }),
  [actionTypes.queue.AQUEDUX_DEQUEUE_NOTIFICATION]: (prevState, _action) => ({
    ...prevState,
  ...
  [actionTypes.queue.AQUEDUX_DEQUEUE_ACTION]: (prevState, _action) => ({
    ...prevState,
    pushQueue: tail(prevState.pushQueue)
  }),
  [actionTypes.queue.AQUEDUX_LOCK_QUEUE]: (prevState, _action) => ({
    ...prevState,
    innerState: 'QUEUE_STATE_BUSY'
  }),
  [actionTypes.queue.AQUEDUX_UNLOCK_QUEUE]: (prevState, _action) => ({
    ...prevState,
